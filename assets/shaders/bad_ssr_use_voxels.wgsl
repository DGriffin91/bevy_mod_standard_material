fn bad_ssr_use_voxels(ifrag_coord: vec2<i32>, surface_normal: vec3<f32>, world_position: vec3<f32>, roughness: f32, F0: vec3<f32>, samples: u32, mip_min_max: vec2<f32>) -> vec4<f32> {
    ...
    for (var i = 0u; i < samples; i += 1u) {
        ...
        let hit = march_voxel_grid(world_position + surface_normal * VOXEL_SIZE, trace_dir_ws, 32u, 0u, 1000.0);
        tot += hit.color * brdf_sample.value_over_pdf;
        /*
        let march_trans = hit.t - 2.0;
        if march_trans > 0.0 {
            tot += hit.color * brdf_sample.value_over_pdf;
        } else {
            ...
            if (raymarch_result.hit) {
                let hit_n = normalize(textureSampleLevel(prepass_downsample, linear_sampler, raymarch_result.hit_uv, lod_rough).xyz);
            ...
            tot += mix(contribution, hit.color, saturate(march_trans + 1.0)) * brdf_sample.value_over_pdf;
        }
        */
    }
    tot /= f32(samples);
