fn bad_ssr(screen_uv: vec2<f32>, ifrag_coord: vec2<i32>, surface_normal: vec3<f32>, world_position: vec3<f32>, roughness: f32, F0: vec3<f32>, sample_index: u32) -> vec4<f32> {
    let samples = 6u;
    ...
    //    );   
        ...
        let brdf_sample = brdf_sample(roughness, F0, tangent_to_world * wo, urand.xy);
        ...
        if (raymarch_result.hit) {
            let hit_n = normalize(prepass_normal(vec4(raymarch_result.hit_uv * view.viewport.zw, 0.0, 0.0), sample_index));
            //let hit_n = normalize(textureSampleLevel(prepass_downsample, linear_sampler, raymarch_result.hit_uv, 0.0).xyz);
            let backface = dot(hit_n, trace_dir_ws);
            if backface < 0.01 {
                let closest_motion_vector = prepass_motion_vector(vec4<f32>(raymarch_result.hit_uv * view.viewport.zw, 0.0, 0.0), sample_index).xy;
                ...
                if history_uv.x > 0.0 && history_uv.x < 1.0 && history_uv.y > 0.0 && history_uv.y < 1.0 {
                    ...
                    let prev_frame = textureSampleLevel(prev_frame_tex, linear_sampler, history_uv, 3.0).rgb;
                    contribution += prev_frame;//prev_frame;//mix(prev_frame, pt_image, raymarch_result.hit_t);
                }
            }
        }
