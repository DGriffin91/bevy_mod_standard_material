...
fn bad_ssgi(ifrag_coord: vec2<i32>, surface_normal: vec3<f32>, world_position: vec3<f32>, samples: u32) -> vec4<f32> {
    let linear_steps = 16u;
    let bisection_steps = 3u;
    ...
    for (var i = 0u; i < samples; i += 1u) {
        ...
        if (raymarch_result.hit) {
            ...
            if backface < 0.01 {
                let closest_motion_vector = prepass_motion_vector(vec4<f32>(raymarch_result.hit_uv * depth_tex_dims, 0.0, 0.0), 0u).xy;
                ...
                if history_uv.x > 0.0 && history_uv.x < 1.0 && history_uv.y > 0.0 && history_uv.y < 1.0 {
                    ...
                    let prev_frame = textureSampleLevel(prev_frame_tex, linear_sampler, history_uv, 0.0).rgb;
