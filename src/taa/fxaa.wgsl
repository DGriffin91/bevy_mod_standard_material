...
fn fxaa(texCoord: vec2<f32>, inverseScreenSize: vec2<f32>) -> vec4<f32> {
    ...
    let lumaDown = rgb2luma(textureSampleLevel(view_target, linear_sampler, texCoord, 0.0, vec2<i32>(0, -1)).rgb);
    let lumaUp = rgb2luma(textureSampleLevel(view_target, linear_sampler, texCoord, 0.0, vec2<i32>(0, 1)).rgb);
    let lumaLeft = rgb2luma(textureSampleLevel(view_target, linear_sampler, texCoord, 0.0, vec2<i32>(-1, 0)).rgb);
    let lumaRight = rgb2luma(textureSampleLevel(view_target, linear_sampler, texCoord, 0.0, vec2<i32>(1, 0)).rgb);
    ...
    let lumaDownLeft  = rgb2luma(textureSampleLevel(view_target, linear_sampler, texCoord, 0.0, vec2<i32>(-1, -1)).rgb);
    let lumaUpRight   = rgb2luma(textureSampleLevel(view_target, linear_sampler, texCoord, 0.0, vec2<i32>(1, 1)).rgb);
    let lumaUpLeft    = rgb2luma(textureSampleLevel(view_target, linear_sampler, texCoord, 0.0, vec2<i32>(-1, 1)).rgb);
    let lumaDownRight = rgb2luma(textureSampleLevel(view_target, linear_sampler, texCoord, 0.0, vec2<i32>(1, -1)).rgb);
    ...
    var lumaEnd1 = rgb2luma(textureSampleLevel(view_target, linear_sampler, uv1, 0.0).rgb);
    var lumaEnd2 = rgb2luma(textureSampleLevel(view_target, linear_sampler, uv2, 0.0).rgb);
    ...
    if (!reachedBoth) {
        for (var i: i32 = 2; i < ITERATIONS; i = i + 1) {
            ...
            if (!reached1) { 
                lumaEnd1 = rgb2luma(textureSampleLevel(view_target, linear_sampler, uv1, 0.0).rgb);
                lumaEnd1 = lumaEnd1 - lumaLocalAverage;
            }
            // If needed, read luma in opposite direction, compute delta.
            if (!reached2) { 
                lumaEnd2 = rgb2luma(textureSampleLevel(view_target, linear_sampler, uv2, 0.0).rgb);
                lumaEnd2 = lumaEnd2 - lumaLocalAverage;
            }
            // If the luma deltas at the current extremities is larger than the local gradient, we have reached the side of the edge.
